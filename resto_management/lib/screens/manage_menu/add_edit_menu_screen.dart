...
/// Layar untuk menambah atau mengedit menu
class AddEditMenuScreen extends StatefulWidget {
  final Map<String, dynamic>?
      menuItem; // Jika null = Add Mode, Jika isi = Edit Mode
...
class _AddEditMenuScreenState extends State<AddEditMenuScreen> {
  ...
  void initState() {
    ...
    _discountController = TextEditingController(
        text: item?['discount_price'] != null
            ? double.parse(item!['discount_price'].toString())
                .toInt()
                .toString()
            : '');
    _stockController =
        TextEditingController(text: item?['stock']?.toString() ?? '20');
  ...
  Future<void> _saveMenu() async {
    ...
    if (_selectedCategoryId == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("Pilih kategori menu!")));
      return;
    }
    ...
    if (discount >= price && discount > 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content:
              Text("Harga diskon tidak boleh lebih besar dari harga asli!"),
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              // Bagian Gambar (Pratinjau Langsung)
              _buildImagePreview(),
              ...
              const SizedBox(height: 16),
              _buildTextField(
                  _descController, "Deskripsi Menu", Icons.description,
                  maxLines: 3),
              const SizedBox(height: 24),

              // Bagian Harga & Stok
              const Text("Harga & Inventori",
                  style: TextStyle(
                      color: Color(0xFFD4AF37),
                      fontWeight: FontWeight.bold,
                      fontSize: 16)),
                ...
                children: [
                  Expanded(
                      child: _buildTextField(
                          _priceController, "Harga (Rp)", Icons.attach_money,
                          isNumber: true)),
                  const SizedBox(width: 16),
                  Expanded(
                      child: _buildTextField(
                          _stockController, "Stok Awal", Icons.inventory_2,
                          isNumber: true)),
                ],
              ),
              const SizedBox(height: 16),
              _buildTextField(_discountController, "Harga Diskon (Opsional)",
                  Icons.discount,
                  isNumber: true, hint: "Kosongkan jika tidak ada diskon"),

              const SizedBox(height: 24),

              // Bagian Pengaturan
              const Text("Pengaturan Status",
                  style: TextStyle(
                      color: Color(0xFFD4AF37),
                      fontWeight: FontWeight.bold,
                      fontSize: 16)),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    SwitchListTile(
                      title: const Text("Status Aktif (Tersedia)",
                          style: TextStyle(color: Colors.white)),
                      subtitle: const Text(
                          "Menu akan tampil di aplikasi pelayan",
                          style:
                    ...
                    SwitchListTile(
                      title: const Text("Menu Promo",
                          style: TextStyle(color: Colors.white)),
                      subtitle: const Text("Tandai sebagai menu spesial/promo",
                          style:
                              TextStyle(color: Colors.white54, fontSize: 12)),
                      value: _isPromo,
            ...
            decoration: BoxDecoration(
              ...
              image: _imageController.text.isNotEmpty
                  ? DecorationImage(
                      ...
                      fit: BoxFit.cover,
                      onError:
                          (exception, stackTrace) {}, // Handle jika link rusak
                    )
                  : null,
            ),
  ...
  Widget _buildTextField(
    ...
    return TextFormField(
      ...
      style: const TextStyle(color: Colors.white),
      validator: (val) {
        if (!isNumber && label.contains("Diskon"))
          return null; // Diskon opsional
